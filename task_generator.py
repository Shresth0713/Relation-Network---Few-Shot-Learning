...
class OmniglotTask(object):
    ...
    def get_class(self, sample):
        # Normalize the path to handle both Unix-like and Windows-like paths
        sample = os.path.normpath(sample)
        # Split the sample path
        parts = sample.split(os.sep)
        # Join all parts except the last one (which is the image filename)
        class_path = os.sep.join(parts[:-1])
        return class_path
